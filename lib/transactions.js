...
export async function dashboardStats() {
  const [today, week, month, year, total, best, unpaidToday, outstanding, latest] = await Promise.all([
    ...
    sql`
      ...
      LIMIT 1
    `,
    sql`SELECT COUNT(*)::int count FROM transactions WHERE transaction_date::date = CURRENT_DATE AND status IN ('Unpaid','With Balance')`,
    sql`
      SELECT COUNT(*)::int count,
             COALESCE(SUM(GREATEST(total_price - amount_paid, 0)), 0) balance
      FROM transactions
      WHERE status IN ('Unpaid','With Balance')
    `,
    sql`SELECT customer_name, id FROM transactions ORDER BY transaction_date DESC LIMIT 1`
  ]);
  ...
  return {
    ...
    unpaidToday: unpaidToday.rows[0]?.count || 0,
    outstanding: outstanding.rows[0],
    latest: latest.rows[0]
  };
}

export async function dashboardChartData() {
  const daily = { labels: [], data: [] };
  for (let i = 29; i >= 0; i--) {
    const result = await sql`
      SELECT COALESCE(SUM(total_price), 0) total
      FROM transactions
      WHERE transaction_date::date = CURRENT_DATE - (${i} * INTERVAL '1 day')
        AND status IN ('Paid','Free','X deal','X Deal')
    `;
    const date = new Date();
    date.setDate(date.getDate() - i);
    daily.labels.push(date.toLocaleDateString('en-US', { month: 'short', day: '2-digit' }));
    daily.data.push(Number(result.rows[0]?.total || 0));
  }
  ...
  for (let i = 11; i >= 0; i--) {
    const result = await sql`
      SELECT COALESCE(SUM(total_price), 0) total
      FROM transactions
      WHERE transaction_date::date >= CURRENT_DATE - ((${i} * 7 + 6) * INTERVAL '1 day')
        AND transaction_date::date <= CURRENT_DATE - ((${i} * 7) * INTERVAL '1 day')
        AND status IN ('Paid','Free','X deal','X Deal')
    `;
    weekly.labels.push(`Week ${i + 1}`);
    weekly.data.push(Number(result.rows[0]?.total || 0));
  }

  const monthly = { labels: [], data: [] };
  for (let i = 11; i >= 0; i--) {
    const result = await sql`
      SELECT COALESCE(SUM(total_price), 0) total
      FROM transactions
      WHERE date_trunc('month', transaction_date) = date_trunc('month', CURRENT_DATE - (${i} * INTERVAL '1 month'))
        AND status IN ('Paid','Free','X deal','X Deal')
    `;
    const date = new Date();
    date.setMonth(date.getMonth() - i);
    monthly.labels.push(date.toLocaleDateString('en-US', { month: 'short', year: 'numeric' }));
    monthly.data.push(Number(result.rows[0]?.total || 0));
  }

  const yearly = { labels: [], data: [] };
  for (let i = 4; i >= 0; i--) {
    const result = await sql`
      SELECT COALESCE(SUM(total_price), 0) total
      FROM transactions
      WHERE date_trunc('year', transaction_date) = date_trunc('year', CURRENT_DATE - (${i} * INTERVAL '1 year'))
        AND status IN ('Paid','Free','X deal','X Deal')
    `;
    const date = new Date();
    date.setFullYear(date.getFullYear() - i);
    yearly.labels.push(String(date.getFullYear()));
    yearly.data.push(Number(result.rows[0]?.total || 0));
  }
  ...
  return {
    periods: {
      daily: { ...daily, title: 'Daily Sales' },
      weekly: { ...weekly, title: 'Weekly Sales' },
      monthly: { ...monthly, title: 'Monthly Sales' },
      yearly: { ...yearly, title: 'Yearly Sales' }
    },
    services: {
      labels: serviceResult.rows.map(row => row.name),
      data: serviceResult.rows.map(row => Number(row.revenue || 0))
    }
  };
}
